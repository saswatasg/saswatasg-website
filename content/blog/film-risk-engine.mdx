...
Most side projects end at "an API collected and a chart made." FilmRisk.AI was different because it has a judgment to render: how risky is a movie's greenlight? Wrong scores are a real cost, so we built-in honesty rails.
...
- **Python 3.11**, scikit-learn (HistGradientBoostingRegressor), PyMC for priors
...
- **Version the backtest data.** Anyone can re-run, but no one can re-fake — pin the sample SHA.
- **Add a "producer mode"** — batch score 50 films at once for slate planning.
