...
By the way — if you're reading this and thinking "sure, but 73% is normal, [Baymard says the industry average is around 70%](https://baymard.com/lists/cart-abandonment-rate)," you're not wrong, and that's exactly the trap. Baymard's aggregate makes 70%+ look like the status quo. It is the status quo. Which is precisely why the aggregate is useless as a lever: nobody acts on a number everyone already agrees with. The moment we split ours into steps, we stopped optimizing a stat and started fixing encounters.
...
Full teardown with the charts and the numbers lives in the [cart & checkout case study](/case-studies/cart-checkout) — and this post is the anchor case behind all my CRO work. The general method is written up in [the data deep-dive](/blog/data-deep-dive-method), and the cadence that shipped these fixes is [one fix a week](/blog/one-fix-a-week-cro).
