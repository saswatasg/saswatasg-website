...
const welcomeMsgs = [
  ...
  "Nice to see you! Let's build.",
  "Portfolio loaded. Let's go!",
];
const clickMsgs = [
...
const idleMsgs = [
  "Looking for best conversion strategies?",
  "Check out my projects!",
  "I ship products that move metrics.",
  "B2B SaaS? D2C? I've done both.",
  "AI agent discovery is my jam.",
  "Want to see my impact stats?",
  "Need help? Just look around!",
  "Still here? Let's build something!",
];
const scrollMsgs = [
  "Great scroll game!",
...
const danceMsgs = [
  "Woo!", "Check this out!", "Get your metrics here!",
  "Let's go!", "This is my jam!",
];
...
const Stickman = () => {
  ...
  const showBubble = useCallback((text, duration = 4000, immediate = false) => {
    const now = Date.now();
    if (!immediate && now - lastBubbleTime.current < 5500) return;
  ...
  useEffect(() => {
    const idle = setInterval(() => {
      if (!bubble.show) {
        showBubble(idleMsgs[Math.floor(Math.random() * idleMsgs.length)], 5000);
      }
    }, 10000);
  ...
  const handleClick = () => {
    ...
    const msg = clickMsgs[clickIdx.current % clickMsgs.length];
    const isOuch = msg === "Ouch!";
    showBubble(msg, 2500, isOuch);
    clickIdx.current += 1;
    lastActive.current = Date.now();
  };
  ...
  const leftEye = expression === 'sleep'
    ? <line x1="27" y1="22" x2="33" y2="22" stroke="#0A0A0A" strokeWidth="1.5" strokeLinecap="round" />
    : <rect x="27" y="20" width="6" height="3" rx="1" fill="#0A0A0A" />;

  const rightEye = expression === 'sleep'
    ? <line x1="39" y1="22" x2="45" y2="22" stroke="#0A0A0A" strokeWidth="1.5" strokeLinecap="round" />
    : <rect x="39" y="20" width="6" height="3" rx="1" fill="#0A0A0A" />;
      ...
      >
        <AnimatePresence>
          {bubble.show && (
            <motion.div
              ...
              className="absolute bottom-[110px] left-1/2 -translate-x-1/2 bg-white border-2 border-black rounded-xl px-3 py-2 text-xs font-bold text-ink max-w-[280px] text-center break-words"
              style={{ boxShadow: '4px 4px 0px 0px #FF90E8' }}
            >
              <div className="absolute -bottom-[7px] left-1/2 -translate-x-1/2 w-3 h-3 bg-white border-r-2 border-b-2 border-black rotate-45" />
              {bubble.text}
            </motion.div>
          )}
        </AnimatePresence>
        ...
        >
          <svg width="72" height="96" viewBox="0 0 72 96" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            >
              <path d="M22 16 Q36 4 50 16 L50 10 Q36 2 22 10 Z" fill="#FF90E8" stroke="#0A0A0A" strokeWidth="2" strokeLinejoin="round" />
              <path d="M48 12 L58 14 L56 11 L48 9 Z" fill="#FF90E8" stroke="#0A0A0A" strokeWidth="2" strokeLinejoin="round" />
            </motion.g>

            <circle cx="36" cy="22" r="13" fill="#F5D6C5" stroke="#0A0A0A" strokeWidth="2" />

            <motion.g
              animate={expression === 'sleep' ? { y: [0, 0, 0] } : quirky === 'peek' ? { x: [0, 8, 0] } : {}}
              transition={{ duration: 0.4 }}
            >
              {leftEye}
              {rightEye}
              <line x1="35" y1="21.5" x2="37" y2="21.5" stroke="#0A0A0A" strokeWidth="1.5" />
            </motion.g>
            ...
            <motion.path
              ...
              strokeLinecap="round"
              fill="none"
              animate={expression === 'open' ? { scaleY: [1, 1.3, 1] } : {}}
              transition={{ duration: 0.3 }}
            />

            <rect x="26" y="35" width="20" height="28" rx="4" fill="#625BF6" stroke="#0A0A0A" strokeWidth="2" />
            <line x1="30" y1="35" x2="28" y2="42" stroke="#625BF6" strokeWidth="3" strokeLinecap="round" />
            <line x1="42" y1="35" x2="44" y2="42" stroke="#625BF6" strokeWidth="3" strokeLinecap="round" />
            ...
            <motion.g
              animate={bored ? { y: [0, 1, 0], transition: { duration: 4, repeat: Infinity } } : {}}
            >
